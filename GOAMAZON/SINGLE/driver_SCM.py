...
case.read('GOAMAZON_SINGLE_DEF_driver.nc')

fin = nc.Dataset('GOAMAZON_SINGLE_DEF_driver.nc','r')
...
ts = np.genfromtxt('sing_sfc_flux_278_5.txt',skip_header=1,dtype=np.float32,usecols=1)
newcase.add_surface_temp(ts,time=timeflux,timeid='time')

# update some attributes
newcase.set_title("Forcing and initial conditions for GOAMAZON/SINGLE case (shallow-to-deep transition) - SCM-enabled version")
newcase.set_script("DEPHY-SCM/GOAMAZON/SINGLE/driver_SCM.py")
...
newcase.write('GOAMAZON_SINGLE_SCM_driver.nc')
