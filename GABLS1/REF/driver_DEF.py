...
## Rodier et al 2020
#From Kosovic and Curry 2000: The initial conditions,surface cooling rate, and the inversion strength for thesesimulations were based on the measurements made dur-ing BASE on 1 October 1994 from flight number 7
#In  the  baseline  simulation,  NLB,  the  latitude  was738N,  the  geostrophic  wind  was  set  to  8  m  s-1,  thesurface cooling rate was 0.25 K h-1, the overlying in-version strength was 0.01 K m-1, and the surface rough-ness was 0.1 m. The baseline case roughness length is higher than the typical roughness length over sea ice inthe Arctic ocean. However, due to the limitations of LESresolution, using a significantly lower roughness lengthwould  result  in  an  underresolved  surface  layer.
...
# 8h duration with a constant coolin
...
#tke initialized to 0 in LES -Meso-NH but 0:4 ð 1  z=250 Þ 3 m 2 s-2 ,for z<250m in Beare et al 2006
...
# Surface Forcing
# constant cooling rate 0.25K/hr from 265K
...
#following flux-gradient relations is recommended
#∂u/dz= ∂v=dz u*/(Kz)*(1.+Bm(z/L))
#∂θ/dz= θ∗/(Kz)*(1+Bh(z/L))
#K=0.4
#Bm=4.8
#Bh=7.8
