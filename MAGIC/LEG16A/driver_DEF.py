...
## MAGIC Leg16A case definition, based on ship-following
...
case = Case('MAGIC/LEG16A',
        startDate="20130803233000",
        endDate="20130807233000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG16A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG16A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/16A/16A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/16A/16A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/16A/16A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,8,3,23,30) # Time of initial radiosonde ascent
...
qlsf=lsf.q.values[8:,:] #kg/kg
tlsf=lsf.T.values[8:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[8:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[8:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[8:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[8:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[8:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG16A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG16A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[8:]
lon = lsf.lon.values[8:]
case.add_latitude(lat,time=lsftime[8:])
case.add_longitude(lon,time=lsftime[8:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[8:,:]
vg=lsf.v_geo.values[8:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[8:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[8:,:] #K/s
rv_adv = lsf.qls[8:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[8:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[8:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[8:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[8:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[8:],levtype='altitude')
...
sst     = sfc.sst[533:] #K
case.add_forcing_ts(sst, time=sfctime[533:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG16A_DEF_driver.nc')
