...
case.read('MAGIC_LEG17A_DEF_driver.nc')
...
# New temporal grid, from 17:45 UTC, 18 August 2013 to 00:15 UTC, 22 August 2013, 30-min timestep
timeout = np.array(range(0,282600,1800),dtype=np.float64)

# conversion
newcase = case.convert2SCM(time=timeout,lev=levout,usetemp=True,levtype='altitude')

# update some attributes
newcase.set_title("Forcing and initial conditions for MAGIC Leg17A case - SCM-enabled version")
newcase.set_script("DEPHY-SCM/MAGIC/LEG17A/driver_SCM.py")
...
newcase.write('MAGIC_LEG17A_SCM_driver.nc')
