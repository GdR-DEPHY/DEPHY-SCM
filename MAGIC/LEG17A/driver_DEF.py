...
## MAGIC Leg17A case definition, based on ship-following
...
case = Case('MAGIC/LEG17A',
        startDate="20130818174500",
        endDate="20130822001500",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG17A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG17A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/17A/17A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/17A/17A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/17A/17A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,8,18,17,45) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2013-08-18 17:46:00
lsftimeorig=lsf.time #hours   since 2013-08-17 20:00:00
sfctimeorig=sfc.time #minutes since 2013-08-17 19:36:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[21].values,lsftimeorig[-1].values)
print(sfctimeorig[121].values,sfctimeorig[-1].values)
...
zlsf=lsf.z.values[21:,:] #m
ulsf=lsf.u.values[21:,:] #m/s
vlsf=lsf.v.values[21:,:] #m/s
qlsf=lsf.q.values[21:,:] #kg/kg
tlsf=lsf.T.values[21:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[21:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[21:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[21:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[21:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[21:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG17A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG17A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[21:]
lon = lsf.lon.values[21:]
case.add_latitude(lat,time=lsftime[21:])
case.add_longitude(lon,time=lsftime[21:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[21:,:]
vg=lsf.v_geo.values[21:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[21:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[21:,:] #K/s
rv_adv = lsf.qls[21:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[21:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[21:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[21:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[21:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[21:],levtype='altitude')
...
sst     = sfc.sst[121:] #K
case.add_forcing_ts(sst, time=sfctime[121:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG17A_DEF_driver.nc')
