...
## MAGIC Leg07A case definition, based on ship-following
...
case = Case('MAGIC/LEG07A',
        startDate="20121117173000",
        endDate="20121122073000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG07A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG07A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/7A/7A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/7A/7A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/7A/7A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2012,12,1,17,30) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2012-12-01 17:30:00
lsftimeorig=lsf.time #hours since 2012-12-01 15:00:00
sfctimeorig=sfc.time #minutes since 2012-12-01 14:32:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[1].values,lsftimeorig[-1].values)#2
print(sfctimeorig[178].values,sfctimeorig[-1].values)#210
...
zlsf=lsf.z.values[1:,:] #m
ulsf=lsf.u.values[1:,:] #m/s
vlsf=lsf.v.values[1:,:] #m/s
qlsf=lsf.q.values[1:,:] #kg/kg
tlsf=lsf.T.values[1:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[1:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[1:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[1:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[1:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[1:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,5.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG07A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG07A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[1:]
lon = lsf.lon.values[1:]
case.add_latitude(lat,time=lsftime[1:])
case.add_longitude(lon,time=lsftime[1:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[1:,:]
vg=lsf.v_geo.values[1:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[1:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[1:,:] #K/s
rv_adv = lsf.qls[1:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[1:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[1:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[1:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[1:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[1:],levtype='altitude')
...
sst     = sfc.sst[178:] #K
case.add_forcing_ts(sst, time=sfctime[178:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG07A_DEF_driver.nc')
