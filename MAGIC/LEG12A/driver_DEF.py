...
## MAGIC Leg12A case definition, based on ship-following
...
# ******NOTE*****
# Observed SST is missing for the first 20 hours of this leg,
# and the SST record contains another significant gap before
# hour 50 of the leg. 
...
case = Case('MAGIC/LEG12A',
        startDate="20130608174500",
        endDate="20130613054500",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG12A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG12A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/12A/12A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/12A/12A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/12A/12A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,6,8,17,45) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2013-06-08 17:45:00
lsftimeorig=lsf.time #hours since 2013-06-08 13:00:00
sfctimeorig=sfc.time #minutes since 2013-06-09 16:50:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[4].values,lsftimeorig[-1].values)
print(sfctimeorig[0].values,sfctimeorig[-1].values)
...
# Model winds disagree strongly with sonde for first 3 profiles,
# don't merge with sonde, but extrapolate sonde to surface.
for istep in range(0,3):
  # Blend model data in with sonde data in lowest 8 levels for winds
  # Leave T and q profiles unchanged
  ubg[istep,5:]=usnd[istep,5:]
  vbg[istep,5:]=vsnd[istep,5:]
  qbg[istep,:]=qsnd[istep,:]
  tbg[istep,:]=tsnd[istep,:]  
  for ilev in range(0,5):
    ubg[istep,ilev]=usnd[istep,5]
    vbg[istep,ilev]=vsnd[istep,5]

for istep in range(3,nsnd):
  # Blend model data in with sonde data in lowest 8 levels for winds
  # Leave T and q profiles unchanged
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG12A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG12A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
#NOTE: SST starts only 20 hours into ship track!
# Pick typical SST value of 288.5C at coast as inital value
sst0 = 288.5#sfc.sst[0].values
...
sst     = sfc.sst[0:] #K
case.add_forcing_ts(sst, time=sfctime[0:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG12A_DEF_driver.nc')
