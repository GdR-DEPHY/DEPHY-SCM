...
## MAGIC Leg11A case definition, based on ship-following
...
case = Case('MAGIC/LEG11A',
        startDate="20130526180000",
        endDate="20130530050000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG11A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG11A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/11A/11A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/11A/11A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/11A/11A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,5,26,18,0) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2013-05-26 18:10:00"
lsftimeorig=lsf.time #hours since 2013-05-25 13:00:00
sfctimeorig=sfc.time #minutes since 2013-05-25 12:56:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[29].values,lsftimeorig[-1].values)
print(sfctimeorig[1744].values,sfctimeorig[-1].values)
...
zlsf=lsf.z.values[29:,:] #m
ulsf=lsf.u.values[29:,:] #m/s
vlsf=lsf.v.values[29:,:] #m/s
qlsf=lsf.q.values[29:,:] #kg/kg
tlsf=lsf.T.values[29:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[29:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[29:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[29:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[29:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[29:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG11A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG11A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[29:]
lon = lsf.lon.values[29:]
case.add_latitude(lat,time=lsftime[29:])
case.add_longitude(lon,time=lsftime[29:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[29:,:]
vg=lsf.v_geo.values[29:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[29:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[29:,:] #K/s
rv_adv = lsf.qls[29:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[29:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[29:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[29:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[29:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[29:],levtype='altitude')
...
sst     = sfc.sst[1744:] #K
case.add_forcing_ts(sst, time=sfctime[1744:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG11A_DEF_driver.nc')
