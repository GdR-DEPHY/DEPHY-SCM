...
## MAGIC Leg06A case definition, based on ship-following
...
case = Case('MAGIC/LEG06A',
        ...
        endDate="20121122060000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG06A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG06A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/6A/6A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/6A/6A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/6A/6A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2012,11,17,17,30) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2012-11-17 17:30:00
lsftimeorig=lsf.time #hours since 2012-11-17 14:00:00
sfctimeorig=sfc.time #minutes since 2012-11-17 14:00:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[2].values,lsftimeorig[-1].values)
print(sfctimeorig[210].values,sfctimeorig[-1].values)
...
zlsf=lsf.z.values[2:,:] #m
ulsf=lsf.u.values[2:,:] #m/s
vlsf=lsf.v.values[2:,:] #m/s
qlsf=lsf.q.values[2:,:] #kg/kg
tlsf=lsf.T.values[2:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[2:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[2:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[2:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[2:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[2:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG06A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG06A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[2:]
lon = lsf.lon.values[2:]
case.add_latitude(lat,time=lsftime[2:])
case.add_longitude(lon,time=lsftime[2:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[2:,:]
vg=lsf.v_geo.values[2:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[2:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[2:,:] #K/s
rv_adv = lsf.qls[2:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[2:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[2:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[2:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[2:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[2:],levtype='altitude')
...
sst     = sfc.sst[210:] #K
case.add_forcing_ts(sst, time=sfctime[210:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG06A_DEF_driver.nc')
