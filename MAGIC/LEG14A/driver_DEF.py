...
## MAGIC Leg14A case definition, based on ship-following
...
case = Case('MAGIC/LEG14A',
        startDate="20130708053000",
        endDate="20130712053000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG14A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG14A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/14A/14A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/14A/14A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/14A/14A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,7,8,5,30) # Time of initial radiosonde ascent (approx)

sndtimeorig=snd.time #minutes since 2013-07-08 05:39:00
lsftimeorig=lsf.time #hours since 2013-07-07 19:00:00
sfctimeorig=sfc.time #minutes since 2013-07-07 21:28:17
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[10].values,lsftimeorig[-1].values)
print(sfctimeorig[482].values,sfctimeorig[-1].values)
...
zlsf=lsf.z.values[10:,:] #m
ulsf=lsf.u.values[10:,:] #m/s
vlsf=lsf.v.values[10:,:] #m/s
qlsf=lsf.q.values[10:,:] #kg/kg
tlsf=lsf.T.values[10:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[10:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[10:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[10:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[10:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[10:], zlsf[:,ilev],
...
for istep in range(0,nsnd):
  ...
  # Sonde with index 4 is an exception: contains bad (interpolated) sonde
  # data over the first 200m. Here, uniformly extrapolate u, v, q profiles 
  # from level 20 to surface, for T use slope of model profile to extrapolate.
  if (istep==4):
      ubg[istep,20:]=usnd[istep,20:]
      vbg[istep,20:]=vsnd[istep,20:]
      qbg[istep,20:]=qsnd[istep,20:]
      tbg[istep,20:]=tsnd[istep,20:]
      for ilev in range(0,20):
        ubg[istep,ilev]=usnd[istep,20]
        vbg[istep,ilev]=vsnd[istep,20]
        qbg[istep,ilev]=qsnd[istep,20]
        tbg[istep,ilev]=tlsf_sonde[istep,ilev]+(tsnd[istep,20]-tlsf_sonde[istep,20])
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG14A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG14A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[10:]
lon = lsf.lon.values[10:]
case.add_latitude(lat,time=lsftime[10:])
case.add_longitude(lon,time=lsftime[10:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[10:,:]
vg=lsf.v_geo.values[10:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[10:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[10:,:] #K/s
rv_adv = lsf.qls[10:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[10:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[10:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[10:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[10:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[10:],levtype='altitude')
...
sst     = sfc.sst[482:] #K
case.add_forcing_ts(sst, time=sfctime[482:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG14A_DEF_driver.nc')
