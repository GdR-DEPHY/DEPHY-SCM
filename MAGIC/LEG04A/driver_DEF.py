...
## MAGIC Leg04A case definition, based on ship-following
...
case = Case('MAGIC/LEG04A',
        startDate="20121020180000",
        endDate="20121025050000",
        surfaceType='ocean',
        zorog=0.)

case.set_title("Forcing and initial conditions for ship-following MAGIC LEG04A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG04A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/4A/4A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/4A/4A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/4A/4A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2012,10,20,18,0) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2012-10-20 17:56:00
lsftimeorig=lsf.time #hours since 2012-10-20 12:00:00
sfctimeorig=sfc.time #minutes since 2012-10-20 12:28:00
print(sndtimeorig[0].values,sndtimeorig[-1].values)
print(lsftimeorig[6].values,lsftimeorig[-1].values)
print(sfctimeorig[332].values,sfctimeorig[-1].values)
...
zlsf=lsf.z.values[6:,:] #m
ulsf=lsf.u.values[6:,:] #m/s
vlsf=lsf.v.values[6:,:] #m/s
qlsf=lsf.q.values[6:,:] #kg/kg
tlsf=lsf.T.values[6:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[6:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[6:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[6:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[6:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[6:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG04A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG04A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[6:]
lon = lsf.lon.values[6:]
case.add_latitude(lat,time=lsftime[6:])
case.add_longitude(lon,time=lsftime[6:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[6:,:]
vg=lsf.v_geo.values[6:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[6:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[6:,:] #K/s
rv_adv = lsf.qls[6:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[6:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[6:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[6:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[6:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[6:],levtype='altitude')
...
sst     = sfc.sst[332:] #K
case.add_forcing_ts(sst, time=sfctime[332:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG04A_DEF_driver.nc')
