...
## MAGIC Leg04A SCM-enabled case definition
...
case.read('MAGIC_LEG04A_DEF_driver.nc')
...
# New temporal grid, from 18:00 UTC, 20 October 2012 to 5:00 UTC, 25 October 2012, 30-min timestep
timeout = np.array(range(0,385200,1800),dtype=np.float64)

# conversion
newcase = case.convert2SCM(time=timeout,lev=levout,usetemp=True,levtype='altitude')

# update some attributes
newcase.set_title("Forcing and initial conditions for MAGIC Leg04A case - SCM-enabled version")
newcase.set_script("DEPHY-SCM/MAGIC/LEG04A/driver_SCM.py")
...
newcase.write('MAGIC_LEG04A_SCM_driver.nc')
