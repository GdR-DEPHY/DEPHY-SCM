...
## MAGIC Leg18A case definition, based on ship-following
...
case.set_title("Forcing and initial conditions for ship-following MAGIC LEG18A case")
case.set_reference("J. McGibbon, C. Bretherton (JAMES 2017)")
case.set_author("M. Ahlgrimm")
case.set_script("DEPHY-SCM/MAGIC/LEG18A/driver_DEF.py")

# radiosonde profiles
snd=xr.open_dataset('../aux/18A/18A_snd.nc')
# large-scale forcing derived from IFS model data
lsf=xr.open_dataset('../aux/18A/18A_lsf.nc')
# SST
sfc=xr.open_dataset('../aux/18A/18A_sfc.nc')

################################################
# 2. Initial state
################################################
basetime=datetime(2013,9,1,5,30) # Time of initial radiosonde ascent

sndtimeorig=snd.time #minutes since 2013-09-01 05:29:00
lsftimeorig=lsf.time #hours since 2013-08-31 13:00:00
sfctimeorig=sfc.time #minutes since 2013-08-31 12:52:00
...
qlsf=lsf.q.values[16:,:] #kg/kg
tlsf=lsf.T.values[16:,:] #k
...
for ilev in range(0,nlevin):
  ff = interpolate.interp1d(lsftime[16:], tlsf[:,ilev],
  ...
  vv = interpolate.interp1d(lsftime[16:], vlsf[:,ilev],
  ...
  uu = interpolate.interp1d(lsftime[16:], ulsf[:,ilev],
  ...
  qq = interpolate.interp1d(lsftime[16:], qlsf[:,ilev],
  ...
  zz = interpolate.interp1d(lsftime[16:], zlsf[:,ilev],
...
if (lblend):
  for tidx in range(0,nsnd): 
    ...
    plt.xlim([-10.,0.])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG18A/images/driver_DEF/merged_background_surface_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
  ...
  #  tidx=10
    ...
    plt.ylim([0,ytop])
    w=w+1
    plt.savefig('/hpc/uhome/mahlgrim/DEPHY-SCM/MAGIC/LEG18A/images/driver_DEF/merged_background_all_'+str(tidx).zfill(2)+'.png')
    plt.close()
  #  plt.show()
...
lat = lsf.lat.values[16:]
lon = lsf.lon.values[16:]
case.add_latitude(lat,time=lsftime[16:])
case.add_longitude(lon,time=lsftime[16:])


# Geostrophic wind
# Use representative mid-track vertical coordinate instead of time-varying coordinate
ug=lsf.u_geo.values[16:,:]
vg=lsf.v_geo.values[16:,:]
case.add_geostrophic_wind(ug=ug,vg=vg,time=lsftime[16:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')

# large-scale horizontal advective tendencies
# Use representative mid-track vertical coordinate instead of time-varying coordinate
t_adv  = lsf.tls[16:,:] #K/s
rv_adv = lsf.qls[16:,:] #kg/kg/s
case.add_rv_advection(rv_adv,time=lsftime[16:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude')
case.add_temp_advection(t_adv,time=lsftime[16:],lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],levtype='altitude',include_rad=False)
...
#plt.plot(lsftime[16:],ps/100.,color='red')
#plt.show()
...
# Use representative mid-track vertical coordinate instead of time-varying coordinate
w=lsf.wls[16:,:] #m/s
case.add_vertical_velocity(w=w,lev=zlsf_sondetime[0,:],height=zlsf_sondetime[0,:],time=lsftime[16:],levtype='altitude')
...
sst     = sfc.sst[998:] #K
case.add_forcing_ts(sst, time=sfctime[998:])

################################################
# 4. Writing file
################################################

case.write('MAGIC_LEG18A_DEF_driver.nc')
