...
t0 = 0     # 18:00 UTC, 26 April 2008
t1 = 28800 # 02:00 UTC, 27 April 2008
...
zuv=825.
inv_tau_nudg_uv=u*0.
inv_tau_nudg_uv[(z>0) & (z<=zuv)]=(1./7200.*(1.-np.cos(np.pi*(z[(z>0) & (z<=zuv)]/zuv)))/2.)
...
inv_tau_nudg_thetal[z>=z1]=(1./3600.*(1.-np.cos(np.pi*(z[z>=z1]/z1)/(z2-z1)))/2.)**(-1)
inv_tau_nudg_thetal[z>z2]=3600.
...
case.add_wind_nudging(unudg=u,vnudg=v,timescale=-1,nudging_constant=inv_tau_nudg_uv,timeid='time',lev=z,levtype='altitude')
...
# nudging time scale tau_nudging  [s]
case.add_thetal_nudging(thetal_nudg,timescale=-1,nudging_constant=inv_tau_nudg_thetal,timeid='time',lev=z,levtype='altitude')

# qt nudging. Set timescale to -1 so that we read vertical profiles of 
# nudging time scale tau_nudging [s]
case.add_qt_nudging(qt_nudg,timescale=-1,nudging_constant=inv_tau_nudg_qt,timeid='time',lev=z,levtype='altitude')
