...
try:
    from metpy.calc import mixing_ratio_from_relative_humidity, relative_humidity_from_mixing_ratio, specific_humidity_from_dewpoint
...
def rt2qt(rt, units='kg kg-1'):
    ...
    if units == 'kg kg-1':
        ...
        logger.error('units unknown: {0}'.format(units))
        raise ValueError('units unknown: {0}'.format(units))
...
#############################
def t2theta(p, temp, p0=cc.p0, kappa=cc.kappa):
    """Compute potential temperature knowing temperature
...
#############################
def z2p(z, ps, thetal=None, theta=None, ta=None,
        qv=None,
...
#############################
def p2z(p, zs=0, thetal=None, theta=None, ta=None,
        qv=None,
        g=cc.g, Rd=cc.Rd, Rv=cc.Rv, p0=cc.p0, kappa=cc.kappa):
    """Compute the altitude of a given set of pressure levels
...
def td2qv(td, pres, units='kg kg-1'):
    ...
    p_loc = pres * Munits.Pa
    td_loc = td * Munits.kelvin
    
    if isinstance(td,float):
        qv = float(specific_humidity_from_dewpoint(p_loc, td_loc))
    else:
        nlev, = td.shape
