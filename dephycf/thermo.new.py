...
def _init_temp(z, T0=300, lapse_rate=-6.5e-3):
    """
    Initialize a temperature profile starting at T0 (300 K by default) at the surface
...
def _init_qv(z, qv0=0.010, Hq=2000.0):
    """
    Initialize a specific humidity profile starting at qv0 (10 g kg-1 by default) at the surface
...
def z2p(
    ...
    In case potential temperature :math:`\\theta` is provided, at level :math:`k`, one
    combines the hydrostatic balance with Poisson's formula
    (:math:`T = \\theta (p/p_0)^\\kappa`) to get:

    .. math:: p_k^\\kappa = p_s^\\kappa - \\kappa p_0^\\kappa
                             \\int_{0}^{z_k} \\frac{g}{R\\theta} dz
    .. math:: p_k^\\kappa \\approx p_s^\\kappa - \\kappa p_0^\\kappa \\times 0.5
                             \\sum_{j=1}^{k} \\left(\\frac{1}{R_{j-1}\\theta_{j-1}}
                             + \\frac{1}{R_j\\theta_j}\\right) g\\, (z_j - z_{j-1})
    .. math:: p_k = \\left(p_k^\\kappa\\right)^{1/\\kappa}
